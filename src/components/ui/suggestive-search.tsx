...
export const TypewriterEffect: React.FC<EffectRendererProps> = ({
  ...
  return (
    ...
    >
      {prefersReducedMotion ? (
        <span className="text-base text-slate-500 select-none">
          {text}
        </span>
      ) : (
        <motion.div
...
export const SlideEffect: React.FC<EffectRendererProps> = ({
  ...
  if (prefersReducedMotion) {
    ...
    return (
      <span className="text-base text-slate-500 select-none">{text}</span>
    );
  }
...
export const SuggestiveSearch: React.FC<SuggestiveSearchProps> = ({
  ...
  className,
  Leading = () => <Search className="size-5 text-slate-400" />,
  showLeading = true,
  Trailing = () => <Search className="size-5 text-slate-400" />,
  showTrailing = false,
  effect = "typewriter",
  EffectComponent,
  typeDurationMs = 500,
  ...
  return (
    <div
      ref={wrapperRef}
      className={cn(
        `relative flex items-center gap-x-2 border border-slate-200 rounded-full bg-white/95 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.4)] ${sizeClasses}`,
        className
      )}
      style={{ maxWidth: "100%" }}
    >
      ...
      <input
        ...
        className={`bg-transparent outline-none text-slate-900 placeholder:text-transparent w-full ${
